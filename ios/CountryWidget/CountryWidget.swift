...
//  CountryWidget
//
//  Created by Aleksandr Kuprienko on 10.03.2021.
//
import WidgetKit
import SwiftUI
import Intents
...
struct Provider: IntentTimelineProvider {
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), flutterData: FlutterData(country: "Hello from Flutter"))
    }

    func getSnapshot(for configuration: ConfigurationIntent, in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        let entry = SimpleEntry(date: Date(), flutterData: FlutterData(country: "Hello from Flutter"))
        completion(entry)
    }
    ...
    func getTimeline(for configuration: ConfigurationIntent, in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        ...
        let sharedDefaults = UserDefaults.init(suiteName: "group.com.akuper")
...
struct FlutterWidgetEntryView : View {
    ...
    private var FlutterDataView: some View {
      Text(entry.flutterData!.country)
    }
    
    private var NoDataView: some View {
      Text("No Country picked! Go to the Flutter App")
    }
